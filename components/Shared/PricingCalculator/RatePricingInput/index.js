...
const RatePricingInput = ({ pricingData, multiple, label, deleteInput, id, parentLabel }) => {
    ...
    switch (e) {
      ...
      case 'subtract': {
        ...
        if (multiple && newValue === 0) {
          ...
          pricingSummaryCopy[productIndex].pricingItems = pricingSummaryCopy[productIndex].pricingItems.filter(subproduct => subproduct.label !== label)
  ...
  useEffect(() => {
    ...
    if (multiple) {
      const subproductIndex = pricingSummaryCopy[productIndex].pricingItems.findIndex(subproduct => subproduct.label === label)
