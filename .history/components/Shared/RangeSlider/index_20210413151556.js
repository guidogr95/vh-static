...
const RangeSlider = React.memo(({ themeColor, min, max, steps, handleSlider, value, indexValues }) => {
    ...
    const indexMin = indexValues ? indexValues[innerMin] : innerMin
    const indexMax = indexValues ? indexValues[innerMax] : innerMax
    const innerStep = typeof steps === 'function' ? steps(value) : typeof steps === 'undefined' ? 1 : steps
    ...
    const sliderPercentage = indexedArray.indexOf(parseInt(value)) / innerMax * 100
    console.log(sliderPercentage, indexedArray, indexedArray.indexOf(parseInt(value)), parseInt(value))
    return (
        <>
