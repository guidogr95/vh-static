...
const BaseLegend = ({ legendData, sliderValue, indexValues, pricingData, label }) => {
  const { productSummary, setProductSummary, productSummaryState } = usePricingCalculator()
  ...
  useEffect(() => {
    setProductSummary(monthlyPricing)
  }, [monthlyPricing])

  return (
    <>
      <div className="base-multiplier__base-legend" >
        <button onClick={() => console.log(productSummary, productSummaryState)} >lp</button>
