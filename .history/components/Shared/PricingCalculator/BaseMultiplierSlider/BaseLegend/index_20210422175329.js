...
const BaseLegend = ({ legendData, sliderValue, indexValues, pricingData, label }) => {
  const { productSummary, setProductSummary } = usePricingCalculator()
  ...
  const findItem = () => {
    const item = productSummary.find(pricingItem => pricingItem.label === label)
    if (!item) {
      console.log(label, productSummary)
      setProductSummary(productSummary.concat({
        label,
        monthlyPricing
      }))
      setTimeout(() => findItem(), 500)
    }
  }

  useEffect(() => {
    // setProductSummary(productSummary.concat([monthlyPricing]))
    // findItem()
  }, [monthlyPricing])
  ...
  return (
    <>
      <div className="base-multiplier__base-legend" >
        <button onClick={() => console.log(productSummary)} >lp</button>
