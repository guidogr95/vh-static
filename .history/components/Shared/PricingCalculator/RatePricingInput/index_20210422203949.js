...
const RatePricingInput = ({ pricingData, multiple, label, deleteInput, id, parentLabel }) => {
  ...
  useEffect(() => {
    if (loaded) return
    const productIndex = pricingSummary.findIndex(product => product.label === parentLabel)
    if (productIndex < 0) return
    const pricingSummaryCopy = Array.from(pricingSummary)
    const subproductIndex = pricingSummaryCopy[productIndex].pricingTable.findIndex(subproduct => subproduct.label === label)
    if (pricingSummaryCopy[productIndex]?.pricingTable[subproductIndex]?.monthlyPricing === undefined) {
      const shouldAdd = subproductIndex === 0 ? true : pricingSummaryCopy[parseInt(productIndex) - 1]?.pricingTable[subproductIndex]?.monthlyPricing !== undefined
      if (shouldAdd) pricingSummaryCopy[productIndex].pricingTable[subproductIndex].monthlyPricing = monthlyPricing
    }
  ...
  // useEffect(() => {
  //   const productIndex = pricingSummary.findIndex(product => product.label === label)
  //   if (productIndex < 0) return
  //   const pricingSummaryCopy = Array.from(pricingSummary)
