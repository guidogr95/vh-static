...
const RatePricingInput = ({ pricingData, multiple, label, deleteInput, id, parentId }) => {
  ...
  useEffect(() => {
    ...
    if (multiple) {
      ...
      if (pricingSummaryCopy[productIndex]?.monthlyPricing === undefined) {
        // This helps avoid maximum depth execution limit and only allows a change of state in order for each product the first time
  ...
  useEffect(() => {
    ...
    if (multiple) {
      ...
      pricingSummaryCopy[productIndex].activeProductId = id
    }
